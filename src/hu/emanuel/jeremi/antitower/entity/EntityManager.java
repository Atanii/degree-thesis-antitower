...
public class EntityManager implements PlayerWorldConnector {
	...
	public EntityManager(Player player, int planeWidth, int planeHeight, 
			MessageProvider rr, Help h, TextureLibrary tl, TowHandler tower, Graphic renderer) {
		this.tower = tower;
		this.tower.gui = this;
	...
	private void LoadLevel() {
		...
		tower.LoadLevel("office.tow", true);
	...
	public BufferedImage getTexture(int id) {
		return texs.getTexture(map.pack, id);
	}
	/*
	private void LoadLevel(String name, MessageProvider rr) {
		this.actualLevelFileName = name;
		
		LevelData d = tower.ReadLevel(texs, name, rr);
	...
	*/
	...
	public void chooseLevel() {
		...
		if (j.showOpenDialog(renderer) == JFileChooser.APPROVE_OPTION) {
			tower.LoadLevel(j.getSelectedFile().getPath(), false);
		} else {
			tower.LoadLevel("office.tow", true);
		}
	}
