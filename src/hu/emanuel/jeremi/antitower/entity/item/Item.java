...
/**
 * This class represents all the usable and assumable items in the game such as
 * weapons, keycards, shields...etc.
 *
 * Since all item types use the same attributes and have the same features,
 * there's no inheritance, nor multiple classes, only the Item class and the
 * type of the item is determined by the value of the type (ItemType enum).
 *
 * The id is simply the id number (or serial number) of the actual item while
 * the "value" is the value used in interactions. For example the value of a
 * weapon is the damage caused by the weapon, the value of a keycard is the code
 * the card has...
 *
