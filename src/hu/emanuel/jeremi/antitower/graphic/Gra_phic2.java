...
public class Graphic2 extends JPanel {
	...
	public final void renderHUDAndOverheadGraphic(Graphics g) {
		g.drawImage(player.getSelectedItem().overheadImg.img , (planeWidth>>1)-(SIZE), planeHeight-(SIZE<<1),SIZE<<1,SIZE<<1, this);
	}
		...
		for( int raysCasted = 0; raysCasted < planeWidth; raysCasted += angleBetweenRays) {
			...
			if(rc.wallDistHorizontal < rc.wallDistVertical) {
				...
				if(rc.DOWN) {
					...
					tex.getTexImg(rc.tileIndexHor).getRGB(rc.offset, 0, 1, SIZE, rc.pixels, 0, 1);
				} else {
					rc.offset = ((((int)(rc.tempAx))) & 63);
					tex.getTexImg(rc.tileIndexHor).getRGB(rc.offset, 0, 1, SIZE, rc.pixels, 0, 1);
				}
				if(rc.storeyIdHor >= 0) {
					tex.getTexImg(rc.storeyIdHor).getRGB(rc.offset, 0, 1, SIZE, rc.storeyPixels, 0, 1);
				}
			// VERTICAL
			} else {
				...
				if(!rc.RIGHT) {
					...
					tex.getTexImg(rc.tileIndexVer).getRGB(rc.offset, 0, 1, SIZE, rc.pixels, 0, 1);
				} else {
					rc.offset = ((((int)(rc.tempAy))) & 63);
					tex.getTexImg(rc.tileIndexVer).getRGB(rc.offset, 0, 1, SIZE, rc.pixels, 0, 1);
				}
				if(rc.storeyIdVer >= 0) {
					tex.getTexImg(rc.storeyIdVer).getRGB(rc.offset, 0, 1, SIZE, rc.storeyPixels, 0, 1);
				}
			}	
			...
			if( rc.v_wallDistHorizontal != Float.MAX_VALUE || rc.v_wallDistVertical != Float.MAX_VALUE ) {
				...
				if( rc.v_wallDistHorizontal < rc.v_wallDistVertical ) {
					...
					if(rc.storeyIdHor >= 0) {
						tex.getTexImg(rc.storeyIdHor).getRGB(rc.v_offset, 0, 1, SIZE, rc.storeyPixels, 0, 1);
					}
				// VERTICAL
				} else {
					rc.isHorizontalWallInside = rc.isVerticalWallInside;
					...
					if(rc.storeyIdVer >= 0) {
						tex.getTexImg(rc.storeyIdVer).getRGB(rc.v_offset, 0, 1, SIZE, rc.storeyPixels, 0, 1);
					}
				}				
			...
			if(floorCastingStartPixel < planeHeight) {
				for (int i=floorCastingStartPixel; i<=planeHeight-1; i++) {
					...
					if( !map.isOutside(mapX, mapY) ) {
						if(IS_SHADERS_ON) {
							screenBuffer.setRGB(raysCasted, i, addFogEffect(tex.textures[map.floorMap[mapY*mapWidth+mapX]].img.getRGB(x&63,y&63),rc.distance));
						} else {
			...
			if(rc.startDraw>=0) {
				for (int i=0; i<=rc.startDraw; i++) {
					...
					try {
						if( !map.isOutside(mapX, mapY) ) {
							if(IS_SHADERS_ON) {
								screenBuffer.setRGB(raysCasted, i, addFogEffect(tex.getTexImg(map.ceilMap[mapY*mapWidth+mapX]).getRGB(x&63,y&63),rc.distance));
							} else {
			...
			*/
			if(rc.isThereThinWallVer || rc.isThereThinWallHor) {
				...
				tex.getTexImg(tex.textures.length-1).getRGB(rc.offset, 0, 1, SIZE, rc.pixels, 0, 1);
		...
		for( int raysCasted = 0; raysCasted < planeWidth; raysCasted += angleBetweenRays) {
			...
			if(wallDistHorizontal < wallDistVertical) {
				...
				if(DOWN) {
					...
					tex.getTexImg(tileIndexHor).getRGB(offset, 0, 1, SIZE, pixels, 0, 1);
				} else {
			...
			// VERTICAL
				...
				if(!RIGHT) {
					...
					tex.getTexImg(tileIndexVer).getRGB(offset, 0, 1, SIZE, pixels, 0, 1);
				} else {
			...
			if(isThereStorey) {
				tex.getTexImg(storeyId).getRGB(offset, 0, 1, SIZE, storeyPixels, 0, 1);
			}			
		    //////////////////////////////////////////////////////////////////////////////////
			...
			*/
			if(isThereThinWallVer || isThereThinWallHor) {
				...
				tex.getTexImg(tex.textures.length-1).getRGB(offset, 0, 1, SIZE, pixels, 0, 1);
	...
	private final void castSprites(BufferedImage screenBuffer) {
		...
		for(int i = 0; i < manager.sprites.length; i++) {
			...
			manager.sprites[i].texture.img.getRGB( 0, 0, SIZE, SIZE, pixels2, 0, SIZE);
...
//	/**
//	 * Inspect the LEFT, UP, DOWN, RIGHT, STEPRIGHT... -- booleans for moving, interaction then
//	 * handles them.
//	 */	
//	public final synchronized void handleInput() {
//		
//		/*
...
//			System.out.println("You're facing right!");
//		else
//			System.out.println("You're facing left!");
//		*/
//		
//		int tempangle = player.angle;
//		if( (tempangle -= ANGLE180) < 0)
//			tempangle += ANGLE360;
//		
//		//System.out.println("\nPlayer's angle: "+player.angle+"\nTemp angle:"+tempangle);
//		
//		if(player.UP) {
//			if( map.isPathWay(((int)((player.x+cosTable[player.angle]*player.speed))>>SIZE_LOG), ((int)((player.y+sinTable[player.angle]*player.speed))>>SIZE_LOG)) ) {
//				player.x += cosTable[player.angle]*player.speed;
//				player.y += sinTable[player.angle]*player.speed;
//			}		
//			if( map.isOutside((player.x>>SIZE_LOG),(player.y>>SIZE_LOG)) ) {
//				player.isInside = false;
//			} else {
...
//		}
//		if(player.DOWN) {
//			if( map.isPathWay(((int)((player.x+cosTable[tempangle]*player.speed))>>SIZE_LOG), ((int)((player.y+sinTable[tempangle]*player.speed))>>SIZE_LOG)) ) {
//				player.x += cosTable[tempangle]*player.speed;
//				player.y += sinTable[tempangle]*player.speed;				
//			}			
//			if( map.isOutside((player.x>>SIZE_LOG),(player.y>>SIZE_LOG)) ) {
//				player.isInside = false;
//			} else {
...
//		}
//		if(player.LEFT) {
//			skybox.rotate(false,player.rotateSpeed);
//			if( (player.angle-=player.rotateSpeed) < ANGLE0 ) {
//				player.angle += ANGLE360;
//			}
//		}
//		if(player.RIGHT) {
//			skybox.rotate(true,player.rotateSpeed);
//			if( (player.angle+=player.rotateSpeed) >= ANGLE360 ) {
//				player.angle -= ANGLE360;
//			}
//		}
//		if(player.STEPLEFT) {
//			int temp = player.angle-ANGLE90;
//			if(temp<0) temp += ANGLE360;
//			
//			if( map.isPathWay(((int)((player.x+cosTable[temp]*player.speed))>>SIZE_LOG), ((int)((player.y+sinTable[temp]*player.speed))>>SIZE_LOG)) ) {
//				player.y += sinTable[temp]*player.speed;
//				player.x += cosTable[temp]*player.speed;
//			}
//			if( map.isOutside((player.x>>SIZE_LOG),(player.y>>SIZE_LOG)) ) {
//				player.isInside = false;
//			} else {
...
//		if(player.STEPRIGHT) {
//			int temp = player.angle+ANGLE90;
//			if(temp>ANGLE360) temp -= ANGLE360;
//			
//			if( map.isPathWay(((int)((player.x+cosTable[temp]*player.speed))>>SIZE_LOG), ((int)((player.y+sinTable[temp]*player.speed))>>SIZE_LOG)) ) {
//				player.y += sinTable[temp]*player.speed;
//				player.x += cosTable[temp]*player.speed;
...
//			manager.makePlayerInteractWithClosestInteractive();
//		}
//		
//		if(player.SHOOTING) {
//			GamePhysicsHelper.traceBeamTillHitsEnemy(
//					100, 
//					(float)player.x+(SIZE>>1), (float)player.y+(SIZE>>1), 
//					cosTable, sinTable, player.angle,
//					manager);
//			renderBeam();
//		}
//		
...
//		setFOV(player.FOV);
//		setPlayerPaneDist(player.playerPaneDist);
//		
//		manager.updateDistBetweenPlayerAndSprites();
//		manager.checkMessagePoint();
//		manager.checkAssumableCollision();
//	}
